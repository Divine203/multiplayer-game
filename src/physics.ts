...
export class Physics {
    ...
    public worldEntities:any = [];

    constructor(game: Game) {
        this.game = game;
        this.game.map.tiles.forEach((tile: Tile, index: number) => {
            const t = Bodies.rectangle(tile.pos.x, tile.pos.y, tile.width, tile.height, { isStatic: true });
            this.worldEntities[index] = t;
         });
 
         this.game.map.items.forEach((item: Item, index: number) => {
             const i = Bodies.rectangle(item.pos.x, item.pos.y, item.width, item.height, { restitution: 0.9 });
             this.worldEntities[4] = i;
         });
        World.add(this.engine.world, this.worldEntities);
    }

    public runEngine() {
        // console.log(this.worldEntities);
